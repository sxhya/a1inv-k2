...
\subsection{Tulenbaev's lifting property}
Let $I \trianglelefteq R$ be an ideal of a commutative ring.
For a nonnilpotent element $a \in R$ denote by $\lambda_a\colon R \rightarrow R_a$ the morphism of principal localization at $a$.
Consider the following commutative square.
\begin{equation} \label{msq}
 \begin{tikzcd} 
    R \ar[r, "\lambda_a"] \ar[d, twoheadrightarrow] \ar[r] & R_a \ar[d, twoheadrightarrow] \\
    R/I \ar[r, "\overline{\lambda_a}"]& R_a/I_a\\
   \end{tikzcd}
\end{equation}
...
The following property of linear Steinberg groups was discovered for the first time by Tulenbaev (see~\cite[Lemmas 2.3, 3.2]{Tu}) and plays a key role in the sequel.
\begin{dfn} \label{def:tlp}
We say that the Steinberg group functor $\St$ satisfies {\it Tulenbaev's lifting property} if for every pull-back square \eqref{msq} the following lifting problem has a solution.
\[\begin{tikzcd} \St(R,   I) \arrow[r, "\mu"] \arrow[d]          &  \St(R) \ar[d, "\lambda_a^*"] \\
                 \St(R_a, I) \arrow[r, "\mu"] \arrow[ur, dotted] &  \St(R_a) \end{tikzcd}\]
\end{dfn}
...
Assume that the Steinberg group functor $\St$ satisfies Tulenbaev's lifting property (cf.~\cref{def:tlp}).
...
Throughout this section $G = G(\Phi, -)$ denotes the (split) simply-connected Chevalley---Demazure group scheme of type $\Phi$.
...
Let $A$ denote arbitrary commutative local ring with the maximal ideal $m$ and the residue field $k$.
Throughout this section we will employ the following notation:
\begin{itemize}
 \item $R$ denotes the Laurent polynomial ring $A[t, t^{-1}]$;
 \item $B^+$ (resp. $B^-$) denotes the subring $A[t] + m[t^{-1}]$ (resp. $m[t] + A[t^{-1}]$) of $R$ consisting of Laurent polynomials $f(t,t^{-1})$ whose 
       coefficients of terms of negative (resp. positive) degree belong to $m$;
 \item $I$ denotes the ideal $m[t, t^{-1}]$ of $R$ (which can be also considered as an ideal of both $B^+$ and $B^-$).
\end{itemize}


Consider the following commutative diagram of groups.
\begin{equation} \label{diag:cs}
\begin{tikzcd} 
C_{+0} \ar[r, hookrightarrow] \ar[d, "k^+", twoheadrightarrow]  & \St(B^+, I) \ar[r, "\mu^{+0}" near start] \ar[d, "j^+"]            & \ar[d, "i^+"] \St(B^+) \ar[r, "p^{+0}", twoheadrightarrow] & \St(k[t]) \ar[d, hookrightarrow, "\overline{i^+}"] \\     
C_{\pm} \ar[r, hookrightarrow]                                  & \St(R, I)   \ar[ur, dashrightarrow, "\varphi"] \ar[r, "\mu^\pm"' near start] & \St(R) \ar[r, "p^\pm"', twoheadrightarrow] & \St(k[t, t^{-1}]) \\
\end{tikzcd} 
\end{equation}
...
The main result of this subsection is the following proposition which is analogous to \cite[Proposition~4.1]{Tu}.
\begin{prop} \label{prop:kersurj} The map $k^+$ is surjective. \end{prop}
\begin{proof}
Compare with \cite[Proposition~4.1]{Tu}.
\end{proof}

Now using a simple diagram chasing argument we derive the main result of this subsection.
\begin{cor} \label{cor:tulinj} The map $i^+$ is injective. \end{cor}
\begin{proof}
