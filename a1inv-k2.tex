...
\usepackage{amssymb, amsxtra, amsmath, amstext, amsthm, amsfonts, amscd, enumitem, hyperref, thmtools,
...
% If the flag below is set to TRUE the bibliography will be processed using BibLaTeX with biber as backend
\newboolean{USEBIBLATEX} \setboolean{USEBIBLATEX}{false}
\ifthenelse{\boolean{USEBIBLATEX}}{  
  \DeclareRobustCommand{\VAN}[2]{#1}
  \usepackage[backend=biber, citestyle=authoryear, style=alphabetic, sortlocale=en_US, url=false, doi=false, eprint=true, firstinits=true, maxbibnames=4]{biblatex}                      
...
\begin{enumerate}
 \item Prove \cref{lem:TulDiagram} for CDE and \cref{lem:TulMapInject}. This should be relatively easy.
...
\begin{lem} \label{lem:TulDiagram}
The canonical maps of Steinberg groups can be completed to the following commutative diagram.
\[\begin{tikzcd} \St(R[t^\pm], m[t^\pm]) \arrow[rr, "i^\pm"] \arrow[dr, dotted] &  & \St(R[t^\pm]) \\
 & \St(R[t^{-1}] + m[t]) \arrow[ur] & \end{tikzcd}\] 
\end{lem}
\begin{proof}
 Compare with \cite[Lemma~3.2]{Tu}. In Tulenbaev's paper this lemma invokes ``another presentation''.
...
In all statements below $R$ denotes a commutative local ring with the maximal ideal $m$ and the residue field $k$.

\begin{prop} \label{prop:TulKernSurject}
The leftmost map in the diagram below is surjective.
\[\begin{tikzcd} 
    \mathrm{Ker}(i^+) \ar[d, twoheadrightarrow] \ar[r, hookrightarrow] & \St(R[t], m[t]) \ar[r, "i^+"] \ar[d] & \ar[d] \St(R[t]) \\
    \mathrm{Ker}(i^\pm) \ar[r, hookrightarrow] & \St(R[t^\pm], m[t^\pm]) \ar[r, "i^\pm"]  & \St(R[t^\pm])
  \end{tikzcd}\]
\end{prop}
\begin{proof}
...
\begin{lem} \label{lem:TulMapInject}
The canonical map $\St(R[t^{-1}]+m[t]) \rightarrow \St(R[t^\pm])$ is injective. \end{lem}
\begin{proof}
 Compare with \cite[Corollary~4.2]{Tu}. This should simultaneously invoke \cref{thm:k[t]}, \cref{lem:TulDiagram} and \cref{prop:TulKernSurject}.
\end{proof}
...
We refer the reader to \cite{Ka}...
