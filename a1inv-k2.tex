...
Obviously, we cannot expect such map to be automorphism or even be defined on the whole $\St(A[t])$.
However, it turns out that in some cases it is still possible to define it on a certain subgroup of $\St(A[t])$.
Indeed, for $\alpha_i\in\Pi$ consider the subgroup $N_i$ of $\St(A[t])$ generated by all $x_\alpha(\xi)$ ($\xi\in A[t]$, $\alpha\in\Phi$) such that 
$\xi$ is divisible by $t$ if $\alpha\in\Sigma_i^-$.

\begin{lem} \label{lem:sigma}
 Assume that for a ring $A$ the canonical embedding $i\colon\Delta_i \hookrightarrow \Phi$ induces an injection $K_2^{G(\Delta_i, -)}(A)\to K_2^{G(\Phi, -)}(A)$.
 Then there exists a homomorphism $\sigma_i' \colon N_i \to \St(A[t])$ compatible with the action of $\sigma_i$, i.\,e. such that the following diagram commutes:
 \[\begin{tikzcd} N_i \arrow[r, "\lambda_t^*"] \arrow[d, "\sigma_i'"]          &  \St(A[t, t^{-1}]) \ar[d, "\sigma_i"] \\
                  \St(A[t]) \arrow[r, "\lambda_t^*"] &  \St(A[t, t^{-1}]) \end{tikzcd}\]

\end{lem}
\begin{proof}
Consider the subgroup $N_i^0$ of $\St(A)$ generated by $x_\alpha(\xi)$ for $\xi \in A$, $\alpha\in\Sigma_i \cup \Delta_i$.
By the Levi decomposition $N_i^0$ is isomorphic to $\UU(\Sigma_i, A) \rtimes \im(\Stb(\Delta_i, A)\rightarrow \Stb(\Phi, A))$.
By our assumption, the right factor in the above product is isomorphic to $\Stb(\Delta_i, A)$, hence $N_i^0 \cong \UU(\Sigma_i, A) \rtimes \Stb(\Delta_i, A)$.

By \cref{item:st-semi} $\St(A[t]) \cong \St(A) \ltimes \St(A, tA[t])$ hence $N_i \cong N_i^0 \ltimes \St(A, tA[t])$.
\end{proof}
