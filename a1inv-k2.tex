...
\[\begin{tikzcd} k^* \cong K_1(k) \ar[r, "{\{t, -\}_\alpha}"] \ar[hookrightarrow, rrd, "{\{t,-\}_\alpha}"'] & K_2(\Phi, k[t^{\pm 1}]) \ar[r] & K_2(\Phi, k(t))\ar[d] \\
...
however, the diagonal map would equal $\{t^2,-\}_\alpha$ and would not be injective. \end{rem}
...
all symbols of the form $\{t,u\}_\alpha$, $\alpha\in\Phi$, $u\in 1+m$. 

Then there is an injective homomorphism $\phi:\Stsymt(\Phi,A,m)\to\Stb(\Phi,A[t^{\pm 1}],I)$, natural in $(A,m)$,
such that the composition of the canonical map 
$\iota\colon\Stb(\Phi,A[t^{\pm 1}],I)\to \Stb(\Phi,A[t^{\pm 1}])$
with $\phi$ equals $\id_{\Stsymt(\Phi,A,m)}$.
\end{lem}
\begin{proof}
