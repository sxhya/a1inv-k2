...
\begin{proof}
 For brevity we write $H$ instead of $\St(A[t]) \times \St(A[t^{-1}])$. 
 Let $(g_+, g_-)$ be an element of $H$ such that $g_+ \cdot g_-^{-1} = 1$ in $\St(A[t, t^{-1}])$.
 
 Let $M$ be a maximal ideal of $A$.
 By \cref{prop:p1g} below the functor $\St(-)$ satisfies $\Pro^1$-glueing for the local ring $A_M$ hence 
 $(\lambda_M^*(g_+), \lambda_M^*(g_-))$ is the image of some element $ g_{0, M} \in \St(A_M)$ under $\Delta_{A_M}$.
 Consequently, in $\St(A_M[t])$ and $\St(A_M[t^{-1}])$ we have equalities...
 $$ ??? $$
 Invoking the local-global principle (see \cref{thm:lg-k2}) we obtain that...
\end{proof}
