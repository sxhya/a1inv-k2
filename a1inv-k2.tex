...
\title{Homotopy invariance for nonstable $K_2$-functors}
...
\begin{enumerate}
\item\label{plan:3t} {\bf (done)} Let $R$ be a local ring. Show that
$$\St(R[t^{\pm 1}])=i_+(\St(R[t]))i_-(\St(R[t^{-1}]))i_+(\St(R[t]))$$
Here $i_\pm$ denote the natural homomorphisms into $\St(R[t^{\pm 1}])$.

\item\label{plan:k[t]} Show that $K_2^G(k[t])=K_2^G(k)$.

\item\label{plan:k-intersect}
...
\item\label{plan:QSlgp} {\bf (done)} Prove the Quillen-Suslin lgp for $K_2^G$. See~\cref{thm:lg-k2}.
...
Let $R$ be arbitrary commutative ring and let $\Phi$ be a root system of type $\rA_\ell, \rC_\ell, \rD_\ell$ or $\rE_\ell$ and rank $\ell \geq 3$. 
An element $g \in \St(\Phi, R[t], tR[t])$ is trivial if and only if 
its image in $\St(\Phi, R_M[t], tR_M[t])$ is trivial for all maximal ideals $M \trianglelefteq R$.
\end{thm}
\begin{proof}
