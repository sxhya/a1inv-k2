...
\end{cor}
\begin{proof}
 From \cref{satz1} it follows that $K_2(\Phi, k)$ is contained in $\WW(\Phi, k)$, therefore the reqired assertion follows from the classical Bruhat decomposition (see e.\,g. \cite[Theorem~4]{St-lect}).
...
\[\begin{tikzcd} 
C_+ \ar[r, hookrightarrow] \ar[dd, "k^+", twoheadrightarrow] & \St(R[t], m[t]) \ar[r, "\mu^+"] \ar[d, "j^+_1"] & \ar[d, "i^+_1"] \St(R[t]) \ar[r, "p^+", twoheadrightarrow] & \St(k[t]) \ar[d, equal] \\    
                                                             & \St(R[t] + m[t^{-1}], m[t^{\pm 1}]) \ar[r, "\mu^{+\varepsilon}" near start] \ar[d, "j^+_2"] & \ar[d, "i^+_2"] \St(R[t]+m[t^{-1}]) \ar[r, "p^{+\varepsilon}", twoheadrightarrow] & \St(k[t]) \ar[d, hookrightarrow, "\overline{i_2^+}"] \\     
C_{\pm} \ar[r, hookrightarrow]                               & \St(R[t^{\pm 1}], m[t^{\pm 1}]) \ar[ur, dashrightarrow, "\varphi"] \ar[r, "\mu^\pm"' near start] & \St(R[t^{\pm 1}]) \ar[r, "p^\pm", twoheadrightarrow] & \St(k[t^{\pm 1}]) \\
\end{tikzcd}\]
...
\begin{proof}
 First of all, notice that by invoking Tulenbaev's lifting property~\ref{def:tlp} we find a map $\varphi$ such that $i_2^+ \varphi = \mu^\pm$.
 
 Now let $g \in \St(R[t] + m[t^{-1}])$ be an element of $\Ker(i_2^+)$.
 ...
 The statement of corollary now follows from a simple diagram chasing, indeed:
 $$g = \varphi(j_2^+(\widetilde{g})) = \varphi(k^+(\widehat{g})) = \varphi(j_2^+(j_1^+(\widehat{g}))) = \mu^{+\varepsilon}(j_1^+(\widehat{g})) = i_1^+(\mu^+(\widehat{g})) = i_1^+(1) = 1. \qedhere $$
\end{proof}
